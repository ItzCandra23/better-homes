...
export namespace HomeForm {
    export function menu(player: ServerPlayer) {
    ...
    export function teleport(player: ServerPlayer) {
        let b: FormButton[] = [];
    ...
    export function add(player: ServerPlayer) {
        new SimpleForm(`§l§8[§2AddHome§8]`, "Select a method.", [
            ...
            if (r === 0) {
                ...
                .sendTo(player.getNetworkIdentifier(), (ff) => {
                    ...
                    if (rr === null) return;
                    HomeMain.createHome(player, rr[0], player.getPosition(), player.getDimensionId());
                });
            }
            if (r === 1) {
                ...
                .sendTo(player.getNetworkIdentifier(), (ff) => {
                    ...
                    if (rr[1] === Number) {
                        if (rr[2] === Number) {
                            if (rr[3] === Number) {
                                HomeMain.createHome(player, rr[0], Vec3.create(r[1], r[2], r[3]), rr[4]);
                            }
                        }
                    }
                });
    ...
    export function remove(player: ServerPlayer) {
        let b: FormButton[] = [];
        const homes = HomeMain.getHomes(player);
        if (!homes) return;
